...
### 4. Recompressão mono em bitrate ajustado à duração, sem cortar conteúdo

`hogwarts.mp3` dura 5:03 (é a faixa inteira, não um trecho curto em loop) e
`centralperk.mp3` dura 46s. Cortar a duração de `hogwarts.mp3` reduziria o tamanho
drasticamente, mas descartaria conteúdo que a proposta não autoriza a remover — a
proposta fala em recomprimir, não em editar. A recompressão preserva a duração das duas
faixas e ajusta o bitrate ao papel de cada uma: 56 kbps mono para a faixa longa de fundo
(resultado: ~2,1 MB, ante 12,1 MB) e 96 kbps mono para a faixa curta (resultado: ~540 KB,
ante 1,8 MB). Isso já não compromete o orçamento de 1 MB do primeiro carregamento, porque
nenhuma das duas é buscada antes da tela de escolha de temporada — o orçamento é sobre o
que baixa até ali, não sobre o tamanho de uma faixa que só é buscada ao entrar na fase que
a usa.

*Alternativa considerada:* cortar `hogwarts.mp3` para um trecho de 60 segundos, comprimido
com fade para disfarçar a costura do loop. Rejeitada por remover conteúdo escolhido pelo
autor sem pedido explícito para tanto; fica registrada aqui como opção caso o tamanho
ainda incomode no futuro.
